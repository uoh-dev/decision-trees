...
function setMeasurement(node: TreeType, updateState: () => void, measurement: string, threshold: number, addLog: (val: string) => void) {
    if (node.type === "leaf") {
    ...
    addLog(`[NODE] added ${measurement} @ ${threshold}`);
    updateState();
}

function setDiagnosis(node: TreeType, updateState: () => void, diagnosis: string | null, addLog: (val: string) => void) {
    if (node.type !== "leaf") return;
    if (diagnosis !== null && diagnosis.length === 0) node.diagnosis = null;
    else node.diagnosis = diagnosis;
    addLog(`[DIAGNOSIS] added ${diagnosis}`);
    updateState();
}

function deleteNode(node: TreeType, updateState: () => void, addLog: (val: string) => void) {
    if (node.type !== "tree") return;
    addLog(`[NODE] deleted ${node.measurement} @ ${node.threshold}`);
...
function NodeAction(props: { node: TreeType, updateState: () => void, measurements: string[], initialLog: string[] }) {
    ...
    const addLog = (val: string) => setLog(log.concat([val]));
    ...
    return <div style={{ display: "flex" }}>
        <div className="node-action">
            <button className="node-action-elem" onClick={() => setMeasurement(props.node, props.updateState, measurement, threshold, addLog)}>Set Measurement</button>
        ...
        <div className="node-action">
            <button className="node-action-elem" disabled={props.node.type === "tree" ? true : false} onClick={() => setDiagnosis(props.node, props.updateState, diagnosis, addLog)}>Set Diagnosis</button>
            <input onChange={(e) => setDiagnosisState(e.target.value)} className="node-action-elem" type="text" value={diagnosis !== null ? diagnosis : ""} />
        </div>
        <div className="node-action">
            <button className="node-action-elem" onClick={() => deleteNode(props.node!, props.updateState, addLog)}>Delete Node</button>
        </div>
    </div>
}
