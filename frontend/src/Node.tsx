...
function Node(props: { node: TreeType, parent: number, updateState: () => void, selector: () => void, isSelected: boolean }) {
    ...
    return <div onClick={props.selector} data-parent={props.parent} className={props.isSelected ? "node node-selected" : "node"} title={"Threshold: " + props.node.threshold.toString()} style={{ textAlign: "center", lineHeight: "3" }}>{props.node.measurement},{props.node.threshold}</div>
}
